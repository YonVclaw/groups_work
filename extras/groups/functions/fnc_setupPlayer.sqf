...
if (_isRespawn) then {
	player setUnitLoadout (missionNamespace getVariable ["LXIF_savedLoadout",_defaultLoadout]);
} else {
	LXIF_savedLoadout = _defaultLoadout;
	...
	} forEach (missionNamespace getVariable ["LXIF_myCustomVariables",[]]);

	LXIF_myCustomVariables = [];
	{
		_x params ["_variable","_value","_global"];
		if (_value in ["true","false"]) then {_value = call compile _value};
		player setVariable [_variable,_value,call compile _global];
		LXIF_myCustomVariables pushBack _variable;
	} forEach _customVariables;

	player setVariable ["LXIF_role",_desiredRole,true];
};
// Team stuff
_playerTColor = player getVariable "lxif_teamcolor";
...
//Name Stuff
call XGRPS_w28fixes_fnc_player_set_name;
