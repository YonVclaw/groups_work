...
if (_type == 28) then {
    if (GVAR(respawnMousePos) isEqualTo -1) then {
        ...
        if (GVAR(respawnMousePos) isEqualType []) then {
            ...
            private _var = missionNamespace getVariable[format["xgrps_respawnPoint%1",GVAR(respawnMousePos)],objNull];
            if (!isNull _var) then {
        ...
        // Close the Map
        ((findDisplay IDC_XGRPS_ADMINMENU_RESP_MAP_DISPLAY) displayCtrl IDC_XGRPS_ADMINMENU_RESP_MAP_CONTROL) ctrlShow false;
        ((findDisplay IDC_XGRPS_ADMINMENU_RESP_MAP_DISPLAY) displayCtrl IDC_XGRPS_ADMINMENU_RESP_MAP_CONTROL) mapCenterOnCamera false;
        closeDialog IDC_XGRPS_ADMINMENU_RESP_MAP_DISPLAY;
    };
};
