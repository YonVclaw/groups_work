...
// Wave Spawners
{
    ...
    _fullmapWindow drawIcon ["\z\xgrps\addons\media\images\logo_256.paa",[0,0,0,0.5],_pos,20,20,0];
...
// Units
{
    if(alive _x) then {
        ...
        if(vehicle _x != _x && crew (vehicle _x) select 0 == _x || vehicle _x == _x) then
        {
            ...
            private _color = (side _x) call xgrps_common_fnc_sideToColor;
...
private _i = 1;
while {true} do {
    private _var = missionNamespace getVariable[format["xgrps_respawnPoint%1",_i],objNull];
