...
if (isNil QGVAR(remoteControlUnits) || {GVAR(remoteControlUnits) isEqualTo []}) exitWith {
    systemChat "[61st Mechanized Infantry Battalion Admin Menu] No unit candidates for remote control";
};
...
_ctrlList ctrlSetPosition [(_dialogPosition # 0) + 0.1 * XGRPS_ADMINMENU_STD_WIDTH, (_dialogPosition # 1) + 0.1 * XGRPS_ADMINMENU_STD_HEIGHT, (_dialogPosition # 2) - (0.2 * XGRPS_ADMINMENU_STD_WIDTH), (_dialogPosition # 3) - (0.2 * XGRPS_ADMINMENU_STD_HEIGHT)];
...
_ctrlOK ctrlSetPosition [(_dialogPosition # 0) + 0.67 * (_dialogPosition # 2), (_dialogPosition # 1) + (_dialogPosition # 3) + 0.1 * XGRPS_ADMINMENU_STD_HEIGHT, 0.33 * (_dialogPosition # 2), XGRPS_ADMINMENU_STD_HEIGHT];
