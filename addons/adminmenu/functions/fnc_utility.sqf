...
if (!isNil QGVAR(selectedTab)) then {
    if (GVAR(selectedTab) isEqualTo IDC_XGRPS_ADMINMENU_G_PMAN && !isNil QGVAR(playerManagement_selected)) then {
        ...
        if ((count GVAR(utilityData)) isEqualTo 0) exitWith {
            systemChat "[61st Mechanized Infantry Battalion Admin Menu] No players selected for the action!";
        };
        ...
        if (_requireAlive) then {
            ...
            if ((count GVAR(utilityData)) isEqualTo 0) exitWith {
                systemChat "[61st Mechanized Infantry Battalion Admin Menu] No alive players selected for the action!";
            };
        };
    };
};
...
if (isNil _utilityFunction) then {
    systemChat format ["[61st Mechanized Infantry Battalion Admin Menu] Utility with name '%1' requires undefined function '%2'", _utilityName, _utilityFunction];
} else {
