...
private _ctrlSpectatorListBox = (_display displayCtrl IDC_XGRPS_ADMINMENU_RESP_SPECTATORLIST);

private _selection = _ctrlSpectatorListBox lbText (lbCurSel _ctrlSpectatorListBox);
private _obj = objNull;
{
    private _name = _x getVariable ["xgrps_spectator_name",name _x];
...
if (!(isNull _obj)) then {
    private _role = lbCurSel (_display displayCtrl IDC_XGRPS_ADMINMENU_RESP_ROLECOMBO); // Role
