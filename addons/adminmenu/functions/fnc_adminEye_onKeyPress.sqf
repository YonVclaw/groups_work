...
if (_type == 20) then {
    if (isNull GVAR(adminEyeSelectedObj)) then {
        ...
        if (typeOf GVAR(adminEyeSelectedObj) == "xgrps_ai_wavespawn") then {
            ...
            if (count _statements > 0) then { // isTrigger.
                [{
                    ...
                    _trigger setVariable ["xgrps_trigger_serialised",[_statements,triggerTimeout _trigger, +_activation]];
                    
                    _activation set[2,false];
                    _trigger setTriggerActivation _activation;
                    _trigger setTriggerStatements ["true","[thisTrigger] call xgrps_adminmenu_fnc_adminEye_restoreTrigger",""];
