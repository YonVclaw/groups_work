...
if (!isMultiplayer) then {
    ...
    } forEach ([0,0,0] nearEntities ["xgrps_spectator_unit",500]);
    {
...
private _control = (_display displayCtrl IDC_XGRPS_ADMINMENU_RESP_FACTIONCATEGORY); /* respawnMenuFactionCategoryCombo */
lbClear _control;
...
{
    private _faction = _x getVariable ["xgrps_assigngear_faction",""];
    if (_faction != "") then {
...
_control = (_display displayCtrl IDC_XGRPS_ADMINMENU_RESP_FACTION); /* respawnMenuFactionCombo */
lbClear _control;
...
_control = (_display displayCtrl IDC_XGRPS_ADMINMENU_RESP_SIDE); /* Side control */
lbClear _control;
...
(_display displayCtrl IDC_XGRPS_ADMINMENU_RESP_SPECTATORTEXT) ctrlSetText format["Players in Spectator: %1",count GVAR(spectatorList) ];

// Marker Type
_control = (_display displayCtrl IDC_XGRPS_ADMINMENU_RESP_MARKERTYPE);
lbClear _control;
{
...
// Marker Colours
_control = (_display displayCtrl IDC_XGRPS_ADMINMENU_RESP_MARKERCOLOUR);
lbClear _control;
{
    private _idx = _control lbAdd (_x select 1);
...
// Group ID
_control = (_display displayCtrl IDC_XGRPS_ADMINMENU_RESP_GROUPNAME);
...
// Marker Name
_control = (_display displayCtrl IDC_XGRPS_ADMINMENU_RESP_GROUPMARKERNAME);
...
if (!isNil QGVAR(respawnGroupMarkerCheckBoxVal)) then {
    (_display displayCtrl IDC_XGRPS_ADMINMENU_RESP_GROUPMARKERCHECKBOX) cbSetChecked GVAR(respawnGroupMarkerCheckBoxVal);
} else {
...
[{
    ...
    _display = findDisplay IDD_XGRPS_ADMINMENU;
    if (isNull (findDisplay IDD_XGRPS_ADMINMENU)) exitWith { [_pfhID] call CBA_fnc_removePerFrameHandler;} ;
