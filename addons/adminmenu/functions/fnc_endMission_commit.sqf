...
// Per-side endings
if (cbChecked (_display displayCtrl IDC_XGRPS_ADMINMENU_ENDM_SIDESPECIFIC)) exitWith {
    private _isDraw = cbChecked (_display displayCtrl IDC_XGRPS_ADMINMENU_ENDM_SIDEDRAW);
    if (_isDraw) then {
...
// Custom text ending
if (cbChecked (_display displayCtrl IDC_XGRPS_ADMINMENU_ENDM_CUSTOM)) exitWith {
    private _title = ctrlText (_display displayCtrl IDC_XGRPS_ADMINMENU_ENDM_CUSTOM_TITLE);
    private _subtext = ctrlText (_display displayCtrl IDC_XGRPS_ADMINMENU_ENDM_CUSTOM_SUBTEXT);
    missionNamespace setVariable [QEGVAR(common,endMissionText), [_title, _subtext], true];

    private _isDefeat = cbChecked (_display displayCtrl IDC_XGRPS_ADMINMENU_ENDM_CUSTOM_ISDEFEAT);
...
private _list = _display displayCtrl IDC_XGRPS_ADMINMENU_ENDM_LIST;
private _ending = _list lbData (lbCurSel _list);
private _isDefeat = cbChecked (_display displayCtrl IDC_XGRPS_ADMINMENU_ENDM_FROMMISSION_ISDEFEAT);
