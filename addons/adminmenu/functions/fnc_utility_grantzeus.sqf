...
if (count _given > 0) then {
    ...
    systemChat format ["[61st Mechanized Infantry Battalion Admin Menu] Zeus access was given to %1", _given];
...
if (count _had > 0) then {
    ...
    systemChat format ["[61st Mechanized Infantry Battalion Admin Menu] Zeus access was already given to %1", _had];
};
