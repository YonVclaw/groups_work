...
if (!isNil "ace_medical_treatment_fnc_fullHeal") then {
    {
        ...
        (format ["[61st Mechanized Infantry Battalion Admin Menu] Your health was restored by '%1'", profileName]) remoteExec ["systemChat", _x];
...
systemChat format ["[61st Mechanized Infantry Battalion Admin Menu] %1 players had their health restored", count GVAR(utilityData)];
