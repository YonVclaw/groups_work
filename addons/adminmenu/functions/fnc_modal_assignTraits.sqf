...
_ctrlLabelTraits ctrlSetPosition [0, 0, (0.25 * _ctrlGrpWidth), XGRPS_ADMINMENU_STD_HEIGHT];
...
private _ctrlComboTraitY = 2.2 * XGRPS_ADMINMENU_STD_HEIGHT;
private _ctrlComboTraitW = 0.5 * _ctrlGrpWidth;
private _traitComboCtrls = [];

{
    private _ctrlLineY = _ctrlComboTraitY + _forEachIndex * (1.1 * XGRPS_ADMINMENU_STD_HEIGHT);
    ...
    _ctrlComboTraitLabel ctrlSetPosition [0.1 * XGRPS_ADMINMENU_STD_WIDTH, _ctrlLineY, _ctrlComboTraitLabelW, XGRPS_ADMINMENU_STD_HEIGHT];
    ...
    _ctrlComboTrait ctrlSetPosition [_ctrlComboTraitX, _ctrlLineY, _ctrlComboTraitW, XGRPS_ADMINMENU_STD_HEIGHT];
...
} forEach ["Medic", "Engineer", "Explosive Specialist", "UAV Hacker", "CAS"];
...
_ctrlButton ctrlAddEventHandler ["ButtonClick", {
    ...
    _traitComboCtrls params ["_comboMed", "_comboEng", "_comboExp", "_comboUAV","_comboCAS"];
    ...
    private _valueCAS = _comboCAS lbValue lbCurSel _comboCAS;

    {
        ...
        if (_valueCAS != -1) then {
            _x setVariable ["lxif_iscom", _valueCAS > 0, true];
        };
    } forEach GVAR(utilityData);

    systemChat format ["[61st Mechanized Infantry Battalion Admin Menu] Assigned traits to %1 player(s)", count GVAR(utilityData)];
