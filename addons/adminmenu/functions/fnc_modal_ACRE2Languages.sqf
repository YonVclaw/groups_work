...
_ctrlLabelLangs ctrlSetPosition [0, 0, (0.5 * _ctrlGrpWidth), XGRPS_ADMINMENU_STD_HEIGHT];
...
if (count acre_sys_core_languages == 0) exitWith {
    ...
    GVAR(utilityTabControls) pushBack _ctrlLabelLangs;
    _ctrlLabelLangs ctrlSetPosition [0, 2.1 * XGRPS_ADMINMENU_STD_HEIGHT, (0.25 * _ctrlGrpWidth), XGRPS_ADMINMENU_STD_HEIGHT];
    _ctrlLabelLangs ctrlCommit 0;
    _ctrlLabelLangs ctrlSetText "No languages configured for mission!";
};

private _langY = 2.1 * XGRPS_ADMINMENU_STD_HEIGHT;
...

{
    ...
    _ctrlLabelName ctrlSetPosition [0, _langY + (_forEachIndex * (1.1 * XGRPS_ADMINMENU_STD_HEIGHT)), (0.25 * _ctrlGrpWidth), XGRPS_ADMINMENU_STD_HEIGHT];
    ...
    _ctrlComboAction ctrlSetPosition [0.5 * _ctrlGrpWidth, _langY + (_forEachIndex * (1.1 * XGRPS_ADMINMENU_STD_HEIGHT)), 0.5 * _ctrlGrpWidth, XGRPS_ADMINMENU_STD_HEIGHT];
...
_ctrlButton ctrlAddEventHandler ["ButtonClick", {
    ...
    if (count _langsToAdd == 0) then {
        if (count _langsToRemove == 0) exitWith {
            systemChat "[61st Mechanized Infantry Battalion Admin Menu] No changes selected";
        };

        systemChat "[61st Mechanized Infantry Battalion Admin Menu] Any change that would leave players with no language to speak will fail";
    };

    [_langsToAdd, _langsToRemove] remoteExecCall [QFUNC(modal_ACRE2Languages_assign), GVAR(utilityData)];

    systemChat format ["[61st Mechanized Infantry Battalion Admin Menu] Assigned ACRE2 languages to %1 player(s)", count GVAR(utilityData)];
