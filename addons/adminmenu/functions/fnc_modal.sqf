...
if (isNil _utilityFunction) exitWith {
    systemChat format ["[61st Mechanized Infantry Battalion Admin Menu] Modal utility with name '%1' requires undefined function '%2'", _utilityName, _utilityFunction];
};

GVAR(utilityData) = [];
private _exit = false;

if ((missionNamespace getVariable [QGVAR(selectedTab), -1]) isEqualTo IDC_XGRPS_ADMINMENU_G_PMAN) then {
...
if (_exit) exitWith {
    systemChat "[61st Mechanized Infantry Battalion Admin Menu] No valid player(s) selected for the action!";
};
