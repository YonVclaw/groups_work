...
private _pfhRefresh = [{
    ...
    (_display displayCtrl IDC_XGRPS_ADMINMENU_DASH_VEHICLES) ctrlSetText str (count vehicles);
    (_display displayCtrl IDC_XGRPS_ADMINMENU_DASH_DEADMEN) ctrlSetText str (count allDeadMen);
    (_display displayCtrl IDC_XGRPS_ADMINMENU_DASH_RUNTIME) ctrlSetText format ["%1m %2s", round ((time - (time % 60)) / 60), round (time % 60)];

    private _ctrlCurators = _display displayCtrl IDC_XGRPS_ADMINMENU_DASH_CURATORS;
    ...
    } forEach IDCS_XGRPS_ADMINMENU_DASH_STATS_ALLSIDES;

    private _numAI = {!isPlayer _x} count _liveUnits;
    (_display displayCtrl IDC_XGRPS_ADMINMENU_DASH_STATS_TOTAL_AI) ctrlSetText str _numAI;

    private _numLiveUnits = count _liveUnits;
    (_display displayCtrl IDC_XGRPS_ADMINMENU_DASH_STATS_TOTAL_PLAYERS) ctrlSetText str (_numLiveUnits - _numAI);

    private _numSpectators = count _spectatorUnits;
    (_display displayCtrl IDC_XGRPS_ADMINMENU_DASH_STATS_TOTAL_SPECTATORS) ctrlSetText str _numSpectators;
    (_display displayCtrl IDC_XGRPS_ADMINMENU_DASH_STATS_TOTAL_TOTAL) ctrlSetText str (_numLiveUnits + _numSpectators);
