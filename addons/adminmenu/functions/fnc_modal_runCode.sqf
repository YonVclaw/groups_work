...
_ctrlEdit ctrlSetPosition [0.1 * XGRPS_ADMINMENU_STD_WIDTH, 1.1 * XGRPS_ADMINMENU_STD_HEIGHT, _ctrlGrpWidth - (0.2 * XGRPS_ADMINMENU_STD_WIDTH), _ctrlGrpHeight - (2.3 * XGRPS_ADMINMENU_STD_HEIGHT)];
...
_ctrlHintEdit ctrlSetPosition [0, 0, _ctrlGrpWidth, XGRPS_ADMINMENU_STD_HEIGHT];
_ctrlHintEdit ctrlCommit 0;
_ctrlHintEdit ctrlSetText "'_this' is the targetted player object";

private _bottomY = _ctrlGrpHeight - XGRPS_ADMINMENU_STD_HEIGHT;

private _ctrlHintCombo = _display ctrlCreate [QGVAR(RscText), -1, _ctrlGroup];
_ctrlHintCombo ctrlSetPosition [0, _bottomY, 0.15 * _ctrlGrpWidth, XGRPS_ADMINMENU_STD_HEIGHT];
...
_ctrlCombo ctrlSetPosition [0.15 * _ctrlGrpWidth, _bottomY, 0.25 * _ctrlGrpWidth, XGRPS_ADMINMENU_STD_HEIGHT];
...
_ctrlButton ctrlSetPosition [0.8 * _ctrlGrpWidth, _bottomY, 0.2 * _ctrlGrpWidth, XGRPS_ADMINMENU_STD_HEIGHT];
...
_ctrlButton ctrlAddEventHandler ["buttonClick", {
    ...
    if (_editText isEqualTo "") then {
        systemChat "[61st Mechanized Infantry Battalion Admin Menu] Code field is empty";
    } else {
        ...
        systemChat format ["[61st Mechanized Infantry Battalion Admin Menu] Code was executed on %1", _ctrlCombo lbText (lbCurSel _ctrlCombo)];
