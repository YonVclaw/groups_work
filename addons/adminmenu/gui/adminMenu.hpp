...
class GVAR(RscButtonMenu): RscButtonMenu {
    ...
    sizeEx = QUOTE(XGRPS_ADMINMENU_STD_SIZEX_L);
...
class GVAR(RscSpectatorControlTableText): RscText {
    sizeEx = QUOTE(XGRPS_ADMINMENU_STD_SIZEX);
    h = "1 * ((((safezoneW / safezoneH) min 1.2) / 1.2) / 25)";
    w = "(6/10) * XGRPS_ADMINMENU_RESP_W_COL1";
    x = 0;
    y = 0;
};
...
class ADDON
{
    ...
    class Controls
    {
        ...
        {
            idc = IDC_XGRPS_ADMINMENU_DASH;
            text = "Dashboard";
            tooltip = "";
            onButtonClick = QUOTE([ARR_2(ctrlParent param [0],IDC_XGRPS_ADMINMENU_G_DASH)] call FUNC(selectTab));
            colorBackground[] = {0, 0, 0, 0};
            x = "1 * (((safezoneW / safezoneH) min 1.2) / 40) + (safezoneX + (safezoneW - ((safezoneW / safezoneH) min 1.2))/2)";
        ...
        {
            idc = IDC_XGRPS_ADMINMENU_PMAN;
            text = "Player Management";
            tooltip = "Perform actions on players";
            onButtonClick = QUOTE([ARR_2(ctrlParent param [0],IDC_XGRPS_ADMINMENU_G_PMAN)] call FUNC(selectTab));
        ...
        // {
        //     idc = IDC_XGRPS_ADMINMENU_RESP;
        //     text = "Respawn";
        //     tooltip = "Respawn dead players back in the game";
        //     onButtonClick = QUOTE([ARR_2(ctrlParent param [0],IDC_XGRPS_ADMINMENU_G_RESP)] call FUNC(selectTab));
        //     x = "16 * (((safezoneW / safezoneH) min 1.2) / 40) + (safezoneX + (safezoneW - ((safezoneW / safezoneH) min 1.2))/2)";
        //     w = "4.8 * (((safezoneW / safezoneH) min 1.2) / 40)";
        // };
        // class TabEndMission: TabDashboard
        // {
        //     idc = IDC_XGRPS_ADMINMENU_ENDM;
        //     text = "End Mission";
        //     tooltip = "Select and execute a mission ending";
        //     onButtonClick = QUOTE([ARR_2(ctrlParent param [0],IDC_XGRPS_ADMINMENU_G_ENDM)] call FUNC(selectTab));
        ...
        {
            idc = IDC_XGRPS_ADMINMENU_MSGS;
            text = "Logs";
            tooltip = "Status and debug messages from 61st Mechanized Infantry Battalion components";
            onButtonClick = QUOTE([ARR_2(ctrlParent param [0],IDC_XGRPS_ADMINMENU_G_MSGS)] call FUNC(selectTab));
        ...
        {
            idc = IDC_XGRPS_ADMINMENU_G_DASH;
            w =  QUOTE(XGRPS_GROUPBASE_W);
            h = QUOTE(XGRPS_GROUPBASE_H);
            #include "dashboard.hpp"
        };

        class GroupPlayerManagement: GroupBase
        {
            idc = IDC_XGRPS_ADMINMENU_G_PMAN;
            w =  QUOTE(XGRPS_GROUPBASE_W);
            h = QUOTE(XGRPS_GROUPBASE_H);
        ...
        {
            idc = IDC_XGRPS_ADMINMENU_G_RESP;
            w =  QUOTE(XGRPS_GROUPBASE_W);
            h = QUOTE(XGRPS_GROUPBASE_H);
            #include "respawn.hpp"
        };
        ...
        {
            idc = IDC_XGRPS_ADMINMENU_G_ENDM;
            w =  QUOTE(XGRPS_GROUPBASE_W);
            h = QUOTE(XGRPS_GROUPBASE_H);
        ...
        {
            idc = IDC_XGRPS_ADMINMENU_G_MSGS;
            w =  QUOTE(XGRPS_GROUPBASE_W);
            h = QUOTE(XGRPS_GROUPBASE_H);
        ...
        {
            idc = IDC_XGRPS_ADMINMENU_UTIL_TBACK;
        ...
        {
            idc = IDC_XGRPS_ADMINMENU_UTIL_TITLE;
            text = "Utility Title";
            style = 0;
            colorBackground[] = {0, 0, 0, 0};
            x = "1 * (((safezoneW / safezoneH) min 1.2) / 40) + (safezoneX + (safezoneW - ((safezoneW / safezoneH) min 1.2))/2)";
        ...
        {
            idc = IDC_XGRPS_ADMINMENU_G_UTIL;
...
class GVAR(respawnMapDialog) {
    idd = IDC_XGRPS_ADMINMENU_RESP_MAP_DISPLAY;
    movingEnable = 0;
    class controlsBackground {
    };
    class objects {
    ...
    class controls {
        ...
        {
            idc = IDC_XGRPS_ADMINMENU_RESP_MAP_CONTROL;
            type = 100;
            ...
            onSetFocus = QUOTE(_this spawn FUNC(respawn_mapLoaded)); //"['respawnMapLoaded'] spawn xgrps_respawn_fnc_handleRespawnUI";
            onMouseButtonDown = QUOTE(_this spawn FUNC(respawn_mapClick)); //"['respawnMap_onMouseButtonDown',_this] spawn xgrps_respawn_fnc_handleRespawnUI";
            onKeyUp = QUOTE(_this spawn FUNC(respawn_mapKeyUp)); //"['respawnMap_keyUp',_this] spawn xgrps_respawn_fnc_handleRespawnUI";
        };
    };
};
...
class GVAR(adminEyeDialog) {
    idd = IDC_XGRPS_ADMINMENU_ADME_MAP_DISPLAY;
    movingEnable = 0;
    class controlsBackground {
    };
    class objects {
    ...
    class controls {
        class FullRespawnMap : RscMapControl
        {
            idc = IDC_XGRPS_ADMINMENU_ADME_MAP_CONTROL;
            type = 100;
            x = QUOTE(0 * safezoneW + safezoneX);
            y = QUOTE(0 * safezoneH + safezoneY);
            w = QUOTE(1 * safezoneW);
