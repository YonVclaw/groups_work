...
private _groupToUpdate = LXIF_dynamicGroups select _groupIndex;
...
private _oldSelectionPath = if (_isRespawn) then {[]} else {[_unit] call FUNC(removeFromGroup)};
...
[_desiredRole,_isRespawn] remoteExecCall [QFDUNC(setupPlayer),_unit];

[_selectedGroup,_unit] remoteExecCall ["selectLeader",groupOwner _selectedGroup];

[LXIF_dynamicGroups,_oldSelectionPath,_selectionPath,_unit] remoteExecCall [QFUNC(updateGroups),-2,"LXIF_DG_JIP"];